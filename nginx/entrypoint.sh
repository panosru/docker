...
envsubst '${CLIENT_MAX_BODY_SIZE},${SERVER_NAME},${FASTCGI_PASS}' < /etc/nginx/conf.d/default.template > /etc/nginx/conf.d/default.conf
...
if [ -d /etc/nginx/custom.conf.d ]; then
    ...
    if [ -n "$CONF_FILES" ]; then
        for f in $CONF_FILES; do
            echo "Found custom Nginx configuration: $f"
        done
    else
